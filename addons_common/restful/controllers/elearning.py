...
class ElearningController(http.Controller):
	...
	def get_completed_slide_slide_partner(self, **payload):
		...
		slide_process_of_user = request.env['slide.channel.partner.line'].search([('slide_id', '=', int(payload.get('slide_id'))), ('slide_channel_partner_id', '=', slide_partner.id)])
		slide_process_of_user.process = 100
